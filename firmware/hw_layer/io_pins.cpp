...
void efiSetPadUnused(brain_pin_e brainPin, int logicValue = -1) {
	(void) logicValue;
#if EFI_PROD_CODE
	...
	if (brain_pin_is_onchip(brainPin)) {
		...
		/* input with pull up, is it safe?
			gggurov: this is NOT safe. add logicValue
        ...
        if (logicValue == -1 ) {
        	...
        	palWritePad(port, pin, logicValue);
        }
