...
void incrementGlobalConfigurationVersion(const char * msg) {
	...
	// engine->module<TpsAccelEnrichment>()->tpsaeburnskip = config->tpsaeburnskipinitial;

	engine->module<TpsAccelEnrichment>()->tpsaeburnskip = config->tpsaeburnskipinitial;

	// prefer to not disrupt engine operation, by not reconfiguring sensors while running
	if (!engineConfiguration->calmerBurn || Sensor::getOrZero(SensorType::Rpm) == 0) {
...
void setConstantDwell(floatms_t dwellMs) {
	...
	setTable(config->dwellRpmVsBatvTable, dwellMs);
}
...
namespace {
    void initTemperatureCurve(
        scaled_channel<signed char, 1, 2>* bins,
        scaled_channel<unsigned char, 50, 1>* values,
        const int size,
    ...
    void initBoostTemperatureCurve(scaled_channel<signed char, 1, 2>* bins, scaled_channel<unsigned char, 50, 1>* values, const float defaultValue) {
...
static void setDefaultEngineConfiguration() {
  ...
  setDefaultCanSettings();
	...
	engineConfiguration->mafMinBufferLength = 1;
...
//	engineConfiguration->idleMode = IM_AUTO;
	...
	engineConfiguration->isMafAveragingEnabled = true;
#endif
