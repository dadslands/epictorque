...
float AirmassVeModelBase::getVe(float rpm, float load, bool postState) const {
	...
	if (postState) {
		// if we're speed density calcs, leave ve and veTableYAxis as is ? maybe switch based on user input ?
		// ve always speed density VE
		if (m_mode == LM_SPEED_DENSITY) {
			engine->engineState.currentVe = ve;
			engine->engineState.veTableYAxis = load;
		} else if (m_mode == LM_REAL_MAF ) {
			engine->engineState.currentMafCorrection = ve;
			engine->engineState.mafAirChargeLoad = load;
		} else {
			engine->engineState.currentVe = ve;
			engine->engineState.veTableYAxis = load;
		}
