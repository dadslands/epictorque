...
float MafAirmass::getMaf() const {
	...
	if (Sensor::hasSensor(SensorType::Maf2) && isAdcChannelValid(engineConfiguration->maf2AdcChannel)) {
...
AirmassResult MafAirmass::getAirmassImpl(float massAirFlow, float rpm, bool postState) const {
	...
	float mafCorrection = getVe(rpm, airChargeLoad, postState);
	mass_t correctedAirmass = cylinderAirmass * mafCorrection;

	engine->engineState.mafAirmass = correctedAirmass;

	if (engine->fuelComputer.targetLambda > 0) {
		engine->outputChannels.tunedMafCorrection = (mafCorrection*100.0f) * (engine->outputChannels.lambdaValue / engine->fuelComputer.targetLambda ) * engine->engineState.stftCorrection[0];
		engine->outputChannels.tunedMassAirflow = massAirFlow * (engine->outputChannels.lambdaValue / engine->fuelComputer.targetLambda ) * engine->engineState.stftCorrection[0];
	}
