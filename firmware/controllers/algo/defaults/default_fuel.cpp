...
static void setDefaultWarmupAFRTargetEnrichment() {
#if CLT_CURVE_SIZE == 8
	static const float bins[] =
	{
		-40,
	...
	copyArray(config->cltAFRTargetCorrBins, bins);
	...
	copyArray(config->cltAFRTargetCorr, values);
#endif // CLT_CURVE_SIZE
}
...
{94.700,	100.900,	103.500,	107.700,	111.800,	115.600,	118.400,	120.300,	120.800,	120.300,	118.700,	116.200,	113.300,	110.500,	107.700,	105.000,	},
	...
	setLinearCurve(config->veLoadBins, 20, 350);
	setLinearCurve(config->veRpmBins, 200, 8000);
	setTable(config->lambdaTable, 0.9);
#endif
	...
	setLinearCurve(engineConfiguration->smallPwInjLookupBins, 0, 2, 0.1);
	setLinearCurve(engineConfiguration->injector.smallPwInjLookup, 0, 2, 0.1);
	setLinearCurve(engineConfiguration->injectorSecondary.smallPwInjLookup, 0, 2, 0.1);
...
static void setDefaultStftSettings() {
	...
	setLinearCurve(config->cltAFRTargetCorrBins, -40, 100, 1);
	static constexpr float cltAfrTargetValues[] = {
		1,1,1,1,1,1,1,1
	};
	copyArray(config->cltAFRTargetCorr, cltAfrTargetValues);

	setRpmTableBin(config->stftTimeConstRpmBins);
	setLinearCurve(config->stftTimeConstLoadBins, 10, 200, 1);
	setTable(config->stftTimeConstTable, 5);
	setTable(config->stftAuthorityAddFuelTable, 5);
	setTable(config->stftAuthorityRemoveFuelTable, 5);
...
#if EFI_UNIT_TEST
static const float tpsTpsTable[TPS_TPS_ACCEL_TABLE][TPS_TPS_ACCEL_TABLE] = {
...
static const float tpsTpsTable[TPS_TPS_ACCEL_TABLE][TPS_TPS_ACCEL_TABLE] = {
/* Generated by TS2C on Tue Apr 18 21:29:16 EDT 2017*/
{/* 0 0	*//* 0 0.0*/0.4,	/* 1 10.0*/0.1,	/* 2 20.0*/0.1,	/* 3 35.0*/0.1,	/* 4 50.0*/0.1,	/* 5 65.0*/0,	/* 6 80.0*/0,	/* 7 100.0*/0,	},
{/* 1 10	*//* 0 0.0*/0.6,	/* 1 10.0*/0.6,	/* 2 20.0*/0.5,	/* 3 35.0*/0.2,	/* 4 50.0*/0.2,	/* 5 65.0*/0,	/* 6 80.0*/0,	/* 7 100.0*/0,	},
{/* 2 20	*//* 0 0.0*/0.8,	/* 1 10.0*/0.9,	/* 2 20.0*/0.8,	/* 3 35.0*/0.4,	/* 4 50.0*/0.4,	/* 5 65.0*/0.2,	/* 6 80.0*/0.1,	/* 7 100.0*/0,	},
{/* 3 35	*//* 0 0.0*/1.0,	/* 1 10.0*/1.0,	/* 2 20.0*/0.9,	/* 3 35.0*/0.6,	/* 4 50.0*/0.6,	/* 5 65.0*/0.3,	/* 6 80.0*/0.2,	/* 7 100.0*/0.1,	},
{/* 4 50	*//* 0 0.0*/1.2,	/* 1 10.0*/1.1,	/* 2 20.0*/1.0,	/* 3 35.0*/0.8,	/* 4 50.0*/0.8,	/* 5 65.0*/0.4,	/* 6 80.0*/0.3,	/* 7 100.0*/0.1,	},
{/* 5 65	*//* 0 0.0*/1.3,	/* 1 10.0*/1.2,	/* 2 20.0*/1.1,	/* 3 35.0*/1,	/* 4 50.0*/0.9,	/* 5 65.0*/0.6,	/* 6 80.0*/0.4,	/* 7 100.0*/0.2,	},
{/* 6 80	*//* 0 0.0*/1.4,	/* 1 10.0*/1.3,	/* 2 20.0*/1.2,	/* 3 35.0*/1.1,	/* 4 50.0*/1,	/* 5 65.0*/0.7,	/* 6 80.0*/0.5,	/* 7 100.0*/0.3,	},
{/* 7 100	*//* 0 0.0*/1.5,	/* 1 10.0*/1.4,	/* 2 20.0*/1.3,	/* 3 35.0*/1.2,	/* 4 50.0*/1,	/* 5 65.0*/0.8,	/* 6 80.0*/0.6,	/* 7 100.0*/0.4,	},
};
#endif

static const  float tauMap[8][8] = {
...
static constexpr float  betaMap[8][8] = {
...
static void setTpsTps() {
	...
	setLinearCurve(config->tpsAeCycleCycleBins, 0, 16, 2);
	setLinearCurve(config->tpsAeCycleTpsToBins, 0, 100, 1);
	setTable(config->tpsAeCycleValues, 0);
...
void setDefaultWallWetting() {
	...
	engineConfiguration->wwaeTau = 0.35;
	engineConfiguration->wwaeBeta = 0.12;
#endif // EFI_UNIT_TEST

	// linear reasonable bins
	setLinearCurve(config->wwCltBins, -40, 100, 1);
	setLinearCurve(config->wwMapBins, 10, 80, 1);
    setLinearCurve(config->wwRpmBins, 500, 2000, 1);
	...
	setRpmTableBin(config->wwRpmBins);
	setLinearCurve(config->wwMapBins, 10, 80, 1);

	copyTable(config->wwTauMapTable, tauMap);
	...
	copyTable(config->wwBetaMapTable, betaMap);
}
...
void setDefaultFuel() {
	...
	// enable acceleration enrichments
	engineConfiguration->useMapEstimateDuringTransient  = 0;
	engineConfiguration->wallWettingAeEnabled = 1;
	engineConfiguration->tpsAccelAeEnabled  = 1;

	engineConfiguration->afrTrimRangeMax = 0.12;
	engineConfiguration->afrTrimRangeMin = -0.12;
	...
	static constexpr float tpsWot[] = {
		100,100,100,100,100,100,100,100,100,100
	};
	copyArray(config->wotTpsValues, tpsWot);
	setRpmTableBin(config->wotTpsRpmBins);
	...
	setTable(config->veTableMaf, 100);
	setTable(config->mafmapBlendValues, 0);
	setLinearCurve(config->mafmapBlendLoadBins, 10, 300);
	setRpmTableBin(config->mafmapBlendRpmBins);
	...
	// clt afr target correction table
	setDefaultWarmupAFRTargetEnrichment();
