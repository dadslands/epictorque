...
void setDefaultBaseEngine() {
  ...
  engineConfiguration->watchOutForLinearTime = true;
	...
	setLinearCurve(config->tpsAeThresholdValue, 10.0, 2.0, 0.1);
  setLinearCurve(engineConfiguration->tractionControlSlipBins, /*from*/0.0, /*to*/2, 0.05);
...
//	engineConfiguration->alsMinPps = 10;
  ...
  // [tag:runNotSquareTest] huh why is this not a unit test?!
	...
	setRpmTableBin(config->tpsAeThresholdRpmBins);
	...
	config->tpsaeburnskipinitial = 15; // this makes burn not freak TPS AE out
