...
angle_t getRunningAdvance(float rpm, float engineLoad) {
  ...
  float wheelSlipFrontRear = Sensor::getOrZero(SensorType::WheelSlipRatioFrontRear);
	...
	engine->outputChannels.tracDisabled = engine->outputChannels.canButtonToggle6;
	engine->ignitionState.tractionAdvanceDrop = 0;
	engine->engineState.tractionControlSparkSkip = 0;

	// traction control button means we don't apply any corrections
	if (engine->outputChannels.canButtonToggle6 != 1 &&                                         // traction enable conditions
      		engineConfiguration->tractionControlEnabled == 1 &&
      		Sensor::getOrZero(SensorType::Rpm) > engineConfiguration->tracMinRpm &&
      		Sensor::getOrZero(SensorType::Map) > engineConfiguration->tracMinMap &&
      		Sensor::getOrZero(SensorType::Tps1) > engineConfiguration->tracMinTps &&
      		vehicleSpeed > engineConfiguration->tracMinSpeed ) {

		if (engineConfiguration->tcUseIgnTiming) {
				engine->ignitionState.tractionAdvanceDrop = 0 - (interpolate3d(
				  engineConfiguration->tractionControlTimingDropLeftRight,
				  engineConfiguration->tractionControlSlipBins, wheelSlipLeftRight,
				  engineConfiguration->tractionControlSpeedBins, vehicleSpeedLeftRight
			  )  +

			  interpolate3d(
				  engineConfiguration->tractionControlTimingDropFrontRear,
				  engineConfiguration->tractionControlSlipBins, wheelSlipFrontRear,
				  engineConfiguration->tractionControlSpeedBins, vehicleSpeedFrontRear
			  )) ;
		}

		if (engineConfiguration->tcUseSparkSkip) {

				engine->engineState.tractionControlSparkSkip = (interpolate3d(
					  engineConfiguration->tractionControlIgnitionSkipLeftRight,
					  engineConfiguration->tractionControlSlipBins, wheelSlipLeftRight,
					  engineConfiguration->tractionControlSpeedBins, vehicleSpeedLeftRight
				  )  +

				  interpolate3d(
					  engineConfiguration->tractionControlIgnitionSkipFrontRear,
					  engineConfiguration->tractionControlSlipBins, wheelSlipFrontRear,
					  engineConfiguration->tractionControlSpeedBins, vehicleSpeedFrontRear
				  )) * 0.01f;
		}
...
floatms_t IgnitionState::getSparkDwell(float rpm, bool isCranking) {
	...
	if (isCranking) {
		...
		dwellMs = interpolate3d(
			config->dwellRpmVsBatvTable,
			config->dwellVoltageCorrVoltBins, Sensor::getOrZero(SensorType::BatteryVoltage),
			config->sparkDwellRpmBins, rpm);

		if (dwellMs < 0.1f) {
			dwellMs = 1.5;
		}

// limit dwell to 90% of duty cycle here:
		if (engineConfiguration->dwellClampPercent < 100) {
			floatms_t engineCycleDuration = getCrankshaftRevolutionTimeMs(rpm) * (getEngineRotationState()->getOperationMode() == TWO_STROKE ? 1 : 2);
			dwellMs = clampF(0, dwellMs, engine->outputChannels.sparkDwellClamp = ((engineConfiguration->dwellClampPercent / 100.0f) * (engineCycleDuration / getNumberOfSparks(getCurrentIgnitionMode() ))));
		}
	}
...
angle_t IgnitionState::getSparkHardwareLatencyCorrection(){
	...
	angle_t correction = interpolate2d(Sensor::getOrZero(SensorType::Rpm), config->sparkDwellRpmBins, engineConfiguration->sparkHardwareLatencyCorrectionCurve) / engine->rpmCalculator.oneDegreeUs;
	engine->outputChannels.SparkLatencyCorrection = correction;
