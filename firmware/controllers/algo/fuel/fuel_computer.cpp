...
float FuelComputer::getStoichiometricRatio() const {
	...
	if (!Sensor::hasSensor(SensorType::FuelEthanolPercent) || ! engineConfiguration->flexEnabled) {
		return primary;
	}
...
float FuelComputer::getTargetLambda(float rpm, float load) const {
	...
	// correct for according to CLT afr multiplier .
...
float IFuelComputer::getLoadOverride(float defaultLoad, load_override_e overrideMode) const {
	switch(overrideMode) {
		...
		case AFR_MAFAirCharge: return engine->engineState.mafAirChargeLoad;
		default: return 0;
	}
}
