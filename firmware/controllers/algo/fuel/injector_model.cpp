...
floatms_t InjectorModelWithConfig::getSmallPulseFlowLookup(floatms_t ms) const {
	engine->outputChannels.stage1InjSmallPwActive = 0;
	engine->outputChannels.stage2InjSmallPwActive = 0;
	if (ms > engineConfiguration->smallPwInjLookupBins[INJ_SMALL_PW_SZ-1]) {
		return ms;
	}

	if (prisec == 1 && !engineConfiguration->useInjSmallPulseWidth1) {
		return ms;
	}

	if (prisec == 2 && !engineConfiguration->useInjSmallPulseWidth2) {
		return ms;
	}

	float newpw = interpolate2d(ms, engineConfiguration->smallPwInjLookupBins, m_cfg->smallPwInjLookup);

	if (prisec == 1) {
		engine->outputChannels.actualLastInjPreSmallPw = ms;
		if (newpw != ms) {
			engine->outputChannels.stage1InjSmallPwActive = 1;
		}
	} else if (prisec == 2)  {
		engine->outputChannels.actualLastInjSTG2PreSmallPw = ms;
		if (newpw != ms) {
			engine->outputChannels.stage2InjSmallPwActive = 1;
		}
	}
	return newpw;
}
...
floatms_t InjectorModelWithConfig::getInjectionDuration(float fuelMassGram) const {
  ...
  // and not more than one of these would stay
	...
	if (getInjectorCompensationMode() != ICM_HPFP_Manual_Compensation) {
		// Add deadtime offset
		return getSmallPulseFlowLookup(baseDuration + m_deadtime);
	}
