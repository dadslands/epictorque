...
void Engine::periodicSlowCallback() {
  ...
  getEngineState()->heaterControlEnabled = engineConfiguration->forceO2Heating || engine->rpmCalculator.isRunning();
	...
	baroLps25Update();
#endif // EFI_PROD_CODE
        if (!engineConfiguration->disableProgOutputs) {
			updateProgrammableOutputs();
        }
...
injection_mode_e getCurrentInjectionMode() {
	// if we're cycling fuel, substitute that reality
	return getEngineRotationState()->isCranking() ? engineConfiguration->crankingInjectionMode :
		( engineConfiguration->dtTuneCycleFuel ? static_cast<injection_mode_e>(engine->outputChannels.dtTuneInjModeOverride) : engineConfiguration->injectionMode );
}
