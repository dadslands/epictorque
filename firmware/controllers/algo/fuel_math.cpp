...
extern ve_Map3D_t veMapMaf;
extern mafmapBlend_t mafmapBlend;
...
//  	float gasoline_density = 0.7429 ;
      float air_density = 1.225; //  g/l
//    this is mg   want_fuel = ((( engineConfiguration->displacement / engineConfiguration->cylindersCount ) * air_density) / engineConfiguration->stoichRatioPrimary) * 1000
// rough fuel calcs, just to start, close enough
// want_fuel = want_fuel / 100 * base fuel
...
//		  ) * 0.001f; // parameter is in milligrams, convert to grams
		...
		baseCrankingFuel =  (((   (    ( engineConfiguration->displacement / engineConfiguration->cylindersCount ) * air_density) / engineConfiguration->stoichRatioPrimary) * 1000.0f / 100.0f)  *  interpolate3d(
			  config->crankingCycleBaseFuel,
			  config->crankingCycleFuelCltBins, Sensor::getOrZero(SensorType::Clt),
			  config->crankingCycleBins, revolutionCounterSinceStart
		  ))  * 0.001f ;
	...
	if (engineConfiguration->flexCranking && Sensor::hasSensor(SensorType::FuelEthanolPercent) && engineConfiguration->flexEnabled) {
...
static MafAirmass mafAirmass(veMapMaf);
static AlphaNAirmass alphaNAirmass(veMap);
static MAFMAPAirmass mafmapAirmass(sdAirmass, mafAirmass, mafmapBlend);
...
AirmassModelBase* getAirmassModel(engine_load_mode_e mode) {
	switch (mode) {
		...
		case LM_MAFMAP:	return &mafmapAirmass;
#if EFI_LUA
