...
ve_Map3D_t veMap{"ve"};
ve_Map3D_t veMapMaf{"veMaf"};
mafmapBlend_t mafmapBlend{"mafmapblend"};
...
void initSpeedDensity() {
	...
	veMapMaf.initTable(config->veTableMaf, config->veRpmBins, config->veLoadBins);
	mafmapBlend.initTable(config->mafmapBlendValues, config->mafmapBlendRpmBins, config->mafmapBlendLoadBins);
}
