...
PUBLIC_API_WEAK void boardTriggerCallback(efitick_t timestamp, float currentPhase) {(void) timestamp;(void) currentPhase;}
...
injection_mode_e dtTuneNextInjectionMode(injection_mode_e cur) {
	switch (cur) {
	case IM_SIMULTANEOUS: // we're in simutaneous -> sequential -> batch
		if (engineConfiguration->dtTuneSequential)    return IM_SEQUENTIAL;
		if (engineConfiguration->dtTuneBatch)         return IM_BATCH;
		if (engineConfiguration->dtTuneSimultaneous)  return IM_SIMULTANEOUS;
		break;

	case IM_SEQUENTIAL: // we're in sequential -> batch -> simultaneous
		if (engineConfiguration->dtTuneBatch)         return IM_BATCH;
		if (engineConfiguration->dtTuneSimultaneous)  return IM_SIMULTANEOUS;
		if (engineConfiguration->dtTuneSequential)    return IM_SEQUENTIAL;
		break;

	case IM_BATCH: // we're in batch  -> simultaneous -> sequential
		if (engineConfiguration->dtTuneSimultaneous)  return IM_SIMULTANEOUS;
		if (engineConfiguration->dtTuneSequential)    return IM_SEQUENTIAL;
		if (engineConfiguration->dtTuneBatch)         return IM_BATCH;
		break;
...
// bit dtTuneSequential;Deadtime Tuning: sequential en
// bit dtTuneBatch;Deadtime Tuning: batch en
// bit dtTuneSimultaneous;Deadtime Tunning: simultaneous en
...
void dtTuneCycle() {
	if (!engineConfiguration->dtTuneCycleFuel ) return;
	if (config->dtTuneCycles == 0) return;

	if (!engineConfiguration->dtTuneSimultaneous &&
		!engineConfiguration->dtTuneSequential &&
		!engineConfiguration->dtTuneBatch) {
		// nothing to do here
		return;
	}

	// running
	if (engine->outputChannels.dtTuneCurCycle > 0) {
		engine->outputChannels.dtTuneCurCycle--;
		return;
	}

	engine->outputChannels.dtTuneCurCycle = config->dtTuneCycles;

	engine->outputChannels.dtTuneInjModeOverride = static_cast<uint8_t>(dtTuneNextInjectionMode(static_cast<injection_mode_e>(engine->outputChannels.dtTuneInjModeOverride)));
}
...
void TriggerCentral::handleShaftSignal(trigger_event_e signal, efitick_t timestamp) {
  ...
  if (boardAllowTriggerActions()) {
	...
	efiPrintf("trigger %d %d %d", triggerIndexForListeners, getRevolutionCounter(), time2print(getTimeNowUs()));
		...
		engine->module<MafAveragingModule>()->triggerCallback(triggerIndexForListeners, timestamp);
		...
		// switch fueling mode on the fly to tune injector deadtime
		if (engine->rpmCalculator.getCachedRpm() > 0 && triggerIndexForListeners == 0 && engineConfiguration->dtTuneCycleFuel) {
			dtTuneCycle();
		}
