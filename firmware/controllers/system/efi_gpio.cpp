...
// default value of -1 means change mode as it was before, if logicValue is given, don't change pin mode, set pin value on the way out
void OutputPin::deInit(int logicValue) {
	(void)logicValue;
...
#if EFI_GPIO_HARDWARE && EFI_PROD_CODE
	efiSetPadUnused(brainPin, logicValue);
#endif /* EFI_GPIO_HARDWARE */
