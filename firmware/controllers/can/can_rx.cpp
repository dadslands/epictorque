...
PUBLIC_API_WEAK void boardProcessCanRxMessage(const size_t busIndex, const CANRxFrame &frame, efitick_t nowNt) { (void) busIndex; (void) nowNt; (void) frame; }


// canButtonBox proxies through executeTSCommand
void processCanButtonBox(const CANRxFrame& frame) {
    uint16_t button;

    if (CAN_ID(frame) != CAN_ECU_BUTTONBOX_RX_ID) {
    	return;
    }
