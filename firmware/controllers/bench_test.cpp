...
void handleButtonBox(uint16_t index) {
	switch(index) {
		...
		case ALL_TRIM_RESET:
            ...
            handleButtonBox(TRAC_TRIM_RESET);
			if (!engineConfiguration->resetCanToggleInhibit1) { engine->outputChannels.canButtonToggle1 = 0;  bitclear(engine->outputChannels.canButtons18, 0 ); }
			if (!engineConfiguration->resetCanToggleInhibit2) { engine->outputChannels.canButtonToggle2 = 0;  bitclear(engine->outputChannels.canButtons18, 1 ); }
			if (!engineConfiguration->resetCanToggleInhibit3) { engine->outputChannels.canButtonToggle3 = 0;  bitclear(engine->outputChannels.canButtons18, 2 ); }
			if (!engineConfiguration->resetCanToggleInhibit4) { engine->outputChannels.canButtonToggle4 = 0;  bitclear(engine->outputChannels.canButtons18, 3 ); }
			if (!engineConfiguration->resetCanToggleInhibit5) { engine->outputChannels.canButtonToggle5 = 0;  bitclear(engine->outputChannels.canButtons18, 4 ); }
			if (!engineConfiguration->resetCanToggleInhibit6) { engine->outputChannels.canButtonToggle6 = 0;  bitclear(engine->outputChannels.canButtons18, 5 ); }
			if (!engineConfiguration->resetCanToggleInhibit7) { engine->outputChannels.canButtonToggle7 = 0;  bitclear(engine->outputChannels.canButtons18, 6 ); }
			if (!engineConfiguration->resetCanToggleInhibit8) { engine->outputChannels.canButtonToggle8 = 0;  bitclear(engine->outputChannels.canButtons18, 7 ); }

		break;

        case CAN_BUTTON_TOGGLE1:
            engine->outputChannels.canButtonToggle1 = !engine->outputChannels.canButtonToggle1;
			if (engine->outputChannels.canButtonToggle1) bitset(engine->outputChannels.canButtons18, 0); else bitclear(engine->outputChannels.canButtons18, 0);
        break;

        case CAN_BUTTON_TOGGLE2:
            engine->outputChannels.canButtonToggle2 = !engine->outputChannels.canButtonToggle2;
			if (engine->outputChannels.canButtonToggle2) bitset(engine->outputChannels.canButtons18, 1); else bitclear(engine->outputChannels.canButtons18, 1);
		break;

        case CAN_BUTTON_TOGGLE3:
            engine->outputChannels.canButtonToggle3 = !engine->outputChannels.canButtonToggle3;
			if (engine->outputChannels.canButtonToggle3) bitset(engine->outputChannels.canButtons18, 2); else bitclear(engine->outputChannels.canButtons18, 2);
        break;

        case CAN_BUTTON_TOGGLE4:
            engine->outputChannels.canButtonToggle4 = !engine->outputChannels.canButtonToggle4;
			if (engine->outputChannels.canButtonToggle4) bitset(engine->outputChannels.canButtons18, 3); else bitclear(engine->outputChannels.canButtons18, 3);
        break;

        case CAN_BUTTON_TOGGLE5:
            engine->outputChannels.canButtonToggle5 = !engine->outputChannels.canButtonToggle5;
			if (engine->outputChannels.canButtonToggle5) bitset(engine->outputChannels.canButtons18, 4); else bitclear(engine->outputChannels.canButtons18, 4);
        break;

        case CAN_BUTTON_TOGGLE6:
            engine->outputChannels.canButtonToggle6 = !engine->outputChannels.canButtonToggle6;
			if (engine->outputChannels.canButtonToggle6) bitset(engine->outputChannels.canButtons18, 5); else bitclear(engine->outputChannels.canButtons18, 5);
        break;

        case CAN_BUTTON_TOGGLE7:
            engine->outputChannels.canButtonToggle7 = !engine->outputChannels.canButtonToggle7;
			if (engine->outputChannels.canButtonToggle7) bitset(engine->outputChannels.canButtons18, 6); else bitclear(engine->outputChannels.canButtons18, 6);
        break;

        case CAN_BUTTON_TOGGLE8:
            engine->outputChannels.canButtonToggle8 = !engine->outputChannels.canButtonToggle8;
			if (engine->outputChannels.canButtonToggle8) bitset(engine->outputChannels.canButtons18, 7); else bitclear(engine->outputChannels.canButtons18, 7);
        break;
		...
		case TRAC_TRIM_UP:
			if (engine->outputChannels.canBoxTracTrim < engineConfiguration->canBoxTracTrimMax) {
				engine->outputChannels.canBoxTracTrim+= engineConfiguration->canBoxTracTrimStep;
			}
		break;

		case TRAC_TRIM_DOWN:
			if (engine->outputChannels.canBoxTracTrim > engineConfiguration->canBoxTracTrimMin) {
				engine->outputChannels.canBoxTracTrim-= engineConfiguration->canBoxTracTrimStep;
			}
		break;

		case TRAC_TRIM_RESET:
			engine->outputChannels.canBoxTracTrim = 0;
		break;

		case TRAC_TRIM_MAX:
			engine->outputChannels.canBoxTracTrim = engineConfiguration->canBoxTracTrimMax;
		break;

		case TRAC_TRIM_MIN:
			engine->outputChannels.canBoxTracTrim = engineConfiguration->canBoxTracTrimMin;
		break;

		case TRAC_TRIM_1:
			engine->outputChannels.canBoxTracTrim = engineConfiguration->canBoxTracTrim1;
		break;

		case TRAC_TRIM_2:
			engine->outputChannels.canBoxTracTrim = engineConfiguration->canBoxTracTrim2;
		break;

		case TRAC_TRIM_3:
			engine->outputChannels.canBoxTracTrim = engineConfiguration->canBoxTracTrim3;
		break;

		case TRAC_TRIM_4:
			engine->outputChannels.canBoxTracTrim = engineConfiguration->canBoxTracTrim4;
		break;
...
PUBLIC_API_WEAK void boardTsAction(uint16_t index) { (void) index; }
