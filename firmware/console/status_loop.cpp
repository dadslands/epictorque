...
static void updateThrottles() {
    ...
    if (Sensor::hasSensor(SensorType::Tps1Secondary)) {
    	engine->outputChannels.tpsSecondaryADC = convertVoltageTo10bitADC(Sensor::getRaw(SensorType::Tps1Secondary));
    }
...
static void updateLambda() {
	...
	// lambdaValue
	if (engine->fuelComputer.targetLambda > 0) {
		engine->outputChannels.afrError = (lambdaValue / engine->fuelComputer.targetLambda );
		engine->outputChannels.tunedVeValue = (engine->outputChannels.veValue) * (lambdaValue / engine->fuelComputer.targetLambda ) * engine->engineState.stftCorrection[0];
	}
...
static void updateRawSensors() {
	...
	engine->outputChannels.rawOilTemperature = Sensor::getRaw(SensorType::OilTemperature);
	...
	engine->outputChannels.rawMaf = Sensor::getRaw(SensorType::MafSlow);
	engine->outputChannels.rawMaf2 = Sensor::getRaw(SensorType::MafSlow2);
	...
	engine->outputChannels.rawAuxAnalog5 = Sensor::getRaw(SensorType::AuxAnalog5);
	engine->outputChannels.rawAuxAnalog6 = Sensor::getRaw(SensorType::AuxAnalog6);
	engine->outputChannels.rawAuxAnalog7 = Sensor::getRaw(SensorType::AuxAnalog7);
	engine->outputChannels.rawAuxAnalog8 = Sensor::getRaw(SensorType::AuxAnalog8);
       // these are auxlinear inputs for not lua
    engine->outputChannels.rawAuxLinear1 = Sensor::getRaw(SensorType::AuxLinear1);
	engine->outputChannels.rawAuxLinear2 = Sensor::getRaw(SensorType::AuxLinear2);
	engine->outputChannels.rawAuxLinear3 = Sensor::getRaw(SensorType::AuxLinear3);
	engine->outputChannels.rawAuxLinear4 = Sensor::getRaw(SensorType::AuxLinear4);
