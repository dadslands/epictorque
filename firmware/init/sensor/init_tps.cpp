...
AdcSubscriptionEntry *adc = nullptr;
	...
	bool init(const TpsConfig& cfg) {
		...
		float lowpasscutoffhz = 200;
		if (!configure(cfg)) {
			return false;
		}

		// if it's pps, sub in ppsLowPassCutoffHz
		if ((m_sens.type() == SensorType::AcceleratorPedalPrimary || m_sens.type()  == SensorType::AcceleratorPedalSecondary) && config->ppsLowPassCutoffHz > 0) {
			lowpasscutoffhz = config->ppsLowPassCutoffHz;
		}

		if ((m_sens.type() == SensorType::Tps1 ||
		m_sens.type()  == SensorType::Tps1Primary ||
		m_sens.type()  == SensorType::Tps1Secondary ||
		m_sens.type()  == SensorType::Tps2 ||
		m_sens.type()  == SensorType::Tps2Primary ||
		m_sens.type()  == SensorType::Tps2Secondary	) && config->tpsLowPassCutoffHz > 0) {
			lowpasscutoffhz = config->tpsLowPassCutoffHz;
		}

		adc = AdcSubscription::SubscribeSensor(m_sens, cfg.channel, /*lowpassCutoffHz*/ lowpasscutoffhz);
