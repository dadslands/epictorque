...
public:
  WheelSlip() : Sensor(SensorType::WheelSlipRatio)  {  }
	...
	SensorResult get() const final override {
  		auto wheelslip = Sensor::get(SensorType::WheelSlipRatioFrontRear);
        return wheelslip.value_or(0);
	}
...
public:
	...
	SensorResult get() const final override {
		...
		engine->outputChannels.wheelspeedFrontAvgError = !FRONTSPEEDAVG.Valid;
		engine->outputChannels.wheelspeedRearAvgError = !REARSPEEDAVG.Valid;


		if (!FRONTSPEEDAVG.Valid && !REARSPEEDAVG.Valid) { // no wheels

			engine->outputChannels.wheelspeedAvgError = 1;
		...
		if (engineConfiguration->drive_train_layout == DRIVETRAIN_RWD || engineConfiguration->drive_train_layout == DRIVETRAIN_AWD) { // slip 1 = front wheel rotating same speed as rear, slip > 1 rear wheel rotating faster, slip < 1 rear wheel rotating slower = LOCKED
	        if (FRONTSPEEDAVG.Valid && REARSPEEDAVG.Valid && FRONTSPEEDAVG.value_or(0) > 0) {	// front and rear present
	        	engine->outputChannels.wheelspeedAvgError = 0;

            	return (REARSPEEDAVG.value_or(0) /FRONTSPEEDAVG.value_or(0)) * (1 + engine->outputChannels.canBoxTracTrim * 0.01f); // scale by canBoxTracTrim
            }
        }
		...
		if (engineConfiguration->drive_train_layout == DRIVETRAIN_FWD) { // slip 1 = front wheel rotating same speed as rear, slip > 1 front wheel rotating faster, slip < 1 front wheel rotating slower = LOCKED
			if (FRONTSPEEDAVG.Valid && REARSPEEDAVG.Valid && REARSPEEDAVG.value_or(0) > 0) {	// front and rear present
				...
				return (FRONTSPEEDAVG.value_or(0) / REARSPEEDAVG.value_or(0)) * (1 + engine->outputChannels.canBoxTracTrim * 0.01f); // scale by canBoxTracTrim
			}
		}
...
public:
	...
	SensorResult get() const final override {
		...
		auto FR = WheelSpeedFR.get();
  		...
  		engine->outputChannels.wheelspeedFLerror = !FL.Valid;
		engine->outputChannels.wheelspeedFRerror = !FR.Valid;
		...
		if (FR.Valid && FL.Valid && FR.value_or(0) > 0) {
			return (FL.value_or(0) / FR.value_or(0))   * (1 + engine->outputChannels.canBoxTracTrim * 0.01f); // scale by canBoxTracTrim
		}
...
public:
	...
	SensorResult get() const final override {
		...
		engine->outputChannels.wheelspeedRLerror = !RL.Valid;
		engine->outputChannels.wheelspeedRRerror = !RR.Valid;
        ...
        if (RR.Valid && RL.Valid && RR.value_or(0) > 0) {
          return (RL.value_or(0) / RR.value_or(0))  * (1 + engine->outputChannels.canBoxTracTrim * 0.01f); // scale by canBoxTracTrim;
        }
		return 1;
	}
	void showInfo(const char*) const override { }
...
void initAuxSpeedSensors() {
  ...
  if (engineConfiguration->enableSlipCalculations) {
