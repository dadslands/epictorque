...
// unfinished/dead?		slowMapSensor2.Register();
        if (!engineConfiguration->noFastMap) { // ignore fast map, let mapCombiner return Sensor::MapSlow
		...
		AdcSubscription::SubscribeSensor(slowMapSensor, mapChannel, config->mapLowPassCutoffHz == 0?100:config->mapLowPassCutoffHz);
	}
