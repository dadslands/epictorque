...
# epicEFI changelog
  ...
  - 4 Wheel Traction Control
    - define all 4 wheels
    - ratios for all 4 wheels
    - left-right intervention
    - front-rear intervention
  - MAFMAP blend
    - separate MAF correction table
    - mafmap blend table  -100% means using maf fuelmass/load calculations, 0% means using speed density
  - MAF averaging and windowing - mirrors MAP averaging and windowing
  - modifyable biquad lowpass hz for inputs (o2, tps, pps, maf, map)
  - fuel level low pass set to 1hz
  - Enable/Disable Fuel pump
  - Enable/Disable injectors coils
  - Enable/Disable ignition coils
  - Injector Deadtime Tuning assist - switches between sequential/batch/simultaneous every set number of engine cycles
  - Major Menu rework
  - CalmerBurn - do not unconfigure all of the sensors and reconfigure them back when clicking burn or closing certain windows.
  - Multiple PPS to TPS maps
  - CANBus Button box configuration and integration
    - AFR trim up/down
    - IDLE UP
    - AC on/off
    - DFCO toggle
    ...
    - Antilag Enable
    - Pedal to TPS map switch
    - traction control adjustments
  - TPS Acceleration enrichment rewrite
  - TPS Acceleration enrichment instant fuel shot
  - Short Term Fuel Trim mappable
    - Mappable Time constant - how fast changes are made
    - Mappable Add fuel authority
    - Mappable Remove fuel authority
  - TunedVE logged - basic adjusted VE from EGO/lambda different logged alongside normal data
  - TunedMafAirflow and TuneMafCorrection - also logged to mimic above
  - Mappable Spark Hardware Latency Correction Curve - RPM to uS adjustment to ignition timing
  - Mappable Dwell Table - RPM vs VOLTS - 3d table
  - Adjustable Dwell Duty Cycle limit - defaults to 90%, will not let dwell be over 90%, on distributor vehicles this will prevent spark cut from ignition coil duty cycle approaching 100%
  - Cranking Base fuel does not use mg of fuel anymore, Value is now basically VE, of % reqfuel
  - Fix cam sync misfire - do not set instant RPM to 0 when cam sync is acquired. This used to misfire before
  - Programmable outputs - allows users to define basic on/off conditions for ports based on ECU variables
  - MCP3208 for epicECU
  - Baro fix for epicECU - used a different chip
  - INJOK for epicECU
  - Accelerometer fix for epicECU
  - Injector Small Pulsewidth is now a simple OLDPW->NEWPW lookup table like MegaSquirt3
  - Target AFR warmup modifier - enables warmup AFR target multiplier for CLT during warmup
