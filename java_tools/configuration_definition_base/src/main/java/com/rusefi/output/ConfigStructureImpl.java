...
public class ConfigStructureImpl implements ConfigStructure {
    ...
    public void addAlignmentFill(ReaderState state, int alignment) {
        ...
        int fillSize = totalSize % alignment == 0 ? 0 : alignment - (totalSize % alignment);
         System.out.println("int fillSize = " + totalSize + "%" + alignment + " == 0 ? 0 : " + alignment + " - ( " + totalSize  + " % " + alignment + ");");
        if (fillSize > 3)
            throw new IllegalStateException("Fill size does not look right: " + fillSize + " total size : " + totalSize);
